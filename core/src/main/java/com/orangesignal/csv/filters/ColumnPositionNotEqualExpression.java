...
/**
 * 指定された項目位置に対応する区切り文字形式データの値が判定基準値と等しくないかどうかでフィルタを適用する区切り文字形式データフィルタの実装です。
 * 
 * @author 杉澤 浩二
 * @since 1.2.3
 */
public class ColumnPositionNotEqualExpression extends ColumnPositionCriteriaExpression {
	 ...
	 */
	protected ColumnPositionNotEqualExpression(final int position, final String criteria) {
	 ...
	 * @throws IllegalArgumentException <code>criteria</code> が <code>null</code> の場合
	 */
	protected ColumnPositionNotEqualExpression(final int position, final String criteria, final boolean ignoreCase) {
