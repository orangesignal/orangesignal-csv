...
final class Type {
	 ...
	 * @return args が types に一致すれば true。<br>違えば flase。
	 */
	static boolean matchFullAll(final Class<?>[] types, final Object[] args) {
	 ...
	 * @return args が types に一致すれば true。<br>違えば flase。
	 */
	static boolean matchRestrictAll(final Class<?>[] types, final Object[] args) {
		boolean match = types.length == args.length;
		for (int i = 0; i < types.length; i++) {
	 ...
	 * @return args が types に一致すれば true。<br>違えば flase。
	 */
	static boolean matchAll(final Class<?>[] types, final Object[] args) {
		boolean match = types.length == args.length;
		for (int i = 0; i < types.length; i++) {
	 ...
	 */
	private static boolean matchFull(final Class<?> type, final Object obj) {
	 ...
	 * @return obj が type の実体であれば true。<br>違えば false。
	 */
	private static boolean matchRestrict(final Class<?> type, final Object obj) {
	...
	private static boolean match(final Class<?> type, final Object obj) {
		...
		if (Type.matchRestrict(type, obj)) {
			...
			return true;
				...
				&& (obj instanceof Number || obj != null
						&& Type.isLongString(obj.toString()) || obj != null
						&& Type.isDoubleString(obj.toString()))) {
			return true;
		} else if ((type.equals(Boolean.TYPE) || type.equals(Boolean.class))
				&& ("TRUE".equalsIgnoreCase(str) || "FALSE"
	 ...
	 * @return 変換後のオブジェクト配列
	 * @exception IllegalAccessError args を types に変換不可能な場合。
	 */
	static Object[] parseAll(final Class<?>[] types, final Object[] args) {
	...
	private static Object parse(final Class<?> type, final Object obj) {
		...
		if (type.isInstance(obj)) {
			...
			return str;
				...
				|| type.equals(Double.class) || type.equals(Double.TYPE))
				&& (obj instanceof Number || obj != null
						&& Type.isLongString(str) || obj != null
						&& Type.isDoubleString(str))) {
			Number num = null;
	 ...
	 * 
	 * @param str 文字列
	 * @return str が確実に Integer を示す文字列なら true。 違えば false。
	 */
	...
	private static boolean isDoubleString(final String str) {
		try {
			...
			if (!num.isInfinite() || str.equals("Infinity")
					|| str.equals("-Infinity")) {
				return true;
			}
			return false;
	 ...
	 * 
	 * @param str 判定対象の文字列
	 * @return str が16進の文字列であれば true。 違えば false。
	 */
	 ...
	 * 
	 * @param str 文字列
	 * @return str を16進数として解釈した値。 str が16進数でない場合の結果は不定。
	 */
