...
public class CsvColumnPositionMappingBeanReader<T> implements Closeable {
	 ...
	 * @throws IllegalArgumentException {@code reader} または {@code type} が {@code null} の場合。
	 */
	public CsvColumnPositionMappingBeanReader(final CsvReader reader, final Class<T> type) {
	 ...
	 * @throws IllegalArgumentException {@code reader} または {@code template} が {@code null} の場合。
	 */
	public CsvColumnPositionMappingBeanReader(final CsvReader reader, final CsvColumnPositionMappingBeanTemplate<T> template) {
		if (reader == null) {
		...
		synchronized (this) {
			if (columnNames == null) {
				...
				if (template.getMaxColumnPosition() == -1) {
					final List<String> names = reader.readValues();
					if (names == null) {
						// ヘッダがない場合は例外をスローします。
				...
				columnNames = Collections.unmodifiableList(template.createColumnNames());
